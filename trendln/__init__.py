...
def plot_sup_res_date(hist, idx, numbest = 2, fromwindows = True, pctbound=0.1,
                      ...
                      errpct = 0.005, hough_scale=0.01, hough_prob_iter=10, sortError=False, accuracy=1,title_txt = "4h"):
    import matplotlib.ticker as ticker
    return plot_support_resistance(hist, ticker.FuncFormatter(datefmt(idx)), numbest, fromwindows,
                                   pctbound, extmethod, method, window, errpct, hough_scale, hough_prob_iter, sortError, accuracy,title_txt=title_txt)
...
def plot_support_resistance(hist, xformatter = None, numbest = 2, fromwindows = True,
                            ...
                            window=125, errpct = 0.005, hough_scale=0.01, hough_prob_iter=10, sortError=False, accuracy=1,title_txt = "4h"):
    import matplotlib.pyplot as plt
    import matplotlib.ticker as ticker
    ...
    plt.title('Support/Resistance Trend Lines in Time frame : ' + title_txt)
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.legend()
    plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(6))
